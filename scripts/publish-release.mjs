 ...
 * internal `@yoopta/*` URLs point at `releases/download/<tag>/…`, so the assets
...
const highlights = ['@yoopta/editor', '@yoopta/mention', '@yoopta/emoji', '@yoopta/paragraph', '@yoopta/themes-shadcn'];
const installBlock = packages
  .filter(({ name }) => highlights.includes(name))
...
All ${packages.length} packages are attached to this release. Copy what you need into
your app's \`package.json\` — the complete block is in
[\`dist-packages/dependencies.json\`](https://github.com/${config.repository}/blob/${config.branch ?? 'master'}/${config.outDir}/dependencies.json).
...
1. **Upgrade every \`@yoopta/*\` entry together.** The tarballs reference each other by
   URL; mixing revisions makes npm install two copies of \`@yoopta/editor\`, and two
   editor instances means broken React context and dead Slate DOM maps.
2. **Never republish a release tag.** npm caches by URL and lockfiles pin an integrity
